                    ...
                    datasets: [
                        {
                            ...
                            data: values,
                            backgroundColor: [documentStyle.getPropertyValue('--blue-500'), documentStyle.getPropertyValue('--green-500'), documentStyle.getPropertyValue('--yellow-500'), documentStyle.getPropertyValue('--red-500'), documentStyle.getPropertyValue('--purple-500')],
                            borderColor: documentStyle.getPropertyValue('--surface-border'),
                            borderWidth: 1
                        }
                    ]
                        ...
                        tooltip: {
                            callbacks: {
                                ...
                                afterLabel: (context) => {
                                    ...
                                    const items = partsMasterResponse.data.filter((part) => `${part.serialNo} - ${part.itemNo}` === groupKey);
                                    const totalItems = items.reduce((acc, part) => acc + (part.quantity || 0), 0);
                                    const totalValue = items.reduce((acc, part) => acc + (part.costAmount || 0) * (part.quantity || 0), 0);

                                    // Generate calculation steps
                                    const calculationSteps = items.map((part) => `RM ${part.costAmount?.toFixed(2) || '0.00'} × ${part.quantity || 0} = RM ${((part.costAmount || 0) * (part.quantity || 0)).toFixed(2)}`).join('\n');

                                    return [`Total Items: ${totalItems}`, `Calculation Steps:`, calculationSteps, `Total Value: RM ${totalValue.toFixed(2)}`].join('\n');
                                }
                            }
                        }
                    },
