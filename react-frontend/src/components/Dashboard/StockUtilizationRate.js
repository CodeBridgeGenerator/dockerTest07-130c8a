    ...
    useEffect(() => {
        ...
        const fetchStockUtilizationRate = async () => {
            try {
                ...
                const totalStockIn = stockInResponse.data.reduce((acc, item) => acc + (item.quantity || 0), 0);
                const totalStockOut = stockOutResponse.data.reduce((acc, item) => acc + (item.quantity || 0), 0);
                        ...
                        tooltip: {
                            callbacks: {
                                ...
                                afterLabel: (context) => {
                                    ...
                                    return [`Usage: ${usage}`, `Stock: ${stock}`, `Utilization: ${((usage / stock) * 100 || 0).toFixed(2)}%`].join('\n');
                                }
                            }
                        }
                    },
    ...
    return (
        <div className="surface-card shadow-2 border-round p-3">
            <div className="flex align-items-center">
                ...
                >
                    <span className="block text-900 font-medium mb-1">Stock Utilization Rate</span>
                </div>
            </div>
            {loading ? (
                ...
                <div>
                    ...
                    >
                        ...
                        <ul>
                            <li>
                                <strong>Stock Utilization Rate:</strong> (Total Stock Out / Total Stock In) × 100
                            </li>
                            <li>
